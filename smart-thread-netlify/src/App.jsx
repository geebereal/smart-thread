...
const REWARD_TASKS = [
  ...
  },
  {
    ...
    desc: "Follow the creator on Threads (daily)",
    reward: 10,
    icon: "✦",
...
export default function SmartThread() {
    ...
    try {
      ...
      if (acc) {
        ...
        saveStore({
          ...
          role: acc.role, giftedPlan: acc.gifted_plan, licenseExpiry: acc.license_expiry,
        });
  ...
  useEffect(() => {
    if (licenseExpiry && new Date(licenseExpiry) < new Date() && plan !== "seed") {
      setPlan("seed");
      setLicenseStatus("none");
      setPlanExpired(true);
  ...
  async function loadAdminStats() {
    ...
    try {
      ...
      setAdminStats({ totalUsers: totalUsers || 0, activeUsers: activeUsers || 0, totalThreads: totalThreads || 0 });
    } catch (e) {
        ...
        {view !== "onboarding" && (
          <header className="fi" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24, paddingTop: 8 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ width: 42, height: 42, borderRadius: 12, background: "linear-gradient(135deg, #d4553a, #e88a3a)", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
                <img src="/logo.png" alt="Smart Thread" style={{ width: 28, height: 28, filter: "brightness(0) invert(1)" }} />
              </div>
              <span style={{ fontFamily: "var(--ui)", fontWeight: 800, fontSize: 20, letterSpacing: "-0.4px" }}>Smart Thread</span>
            </div>
            <nav style={{ display: "flex", gap: 3 }}>
              {[
                ...
                <button key={tab.v} onClick={tab.onClick} style={{
                  padding: "8px 16px", borderRadius: 8, fontSize: 14, fontFamily: "var(--ui)", fontWeight: 500,
          ...
          <div className="fi">
            ...
            {history.length === 0 ? (
              ...
              <div key={h.id} className="glass" style={{ borderRadius: 12, padding: 16, marginBottom: 8 }}>
                ...
                {/* First tweet preview */}
                {h.tweets?.[0] && (
                  <p style={{ fontSize: 12, fontFamily: "var(--ui)", color: "var(--sub)", lineHeight: 1.5, marginBottom: 10, fontStyle: "italic" }}>"{h.tweets[0].slice(0, 100)}{h.tweets[0].length > 100 ? "..." : ""}"</p>
                )}
                    ...
                    <div key={task.id} style={{
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        ...
                        <div style={{
                          ...
                          {isDone ? "Done today ✓" : isPending ? "Pending review" : `+${task.reward} gens/day`}
                        </div>
                      </div>
            ...
            {isAdmin && (
              <div className="glass settings-section" style={{ borderColor: "rgba(212,85,58,0.3)" }}>
                ...
                {adminClaimedKeys.length === 0 ? (
                  <p style={{ fontSize: 12, fontFamily: "var(--ui)", color: "var(--sub)", fontStyle: "italic" }}>No keys generated yet.</p>
                ) : (
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {adminClaimedKeys.map(k => (
                      <div key={k.id} style={{
                        padding: "10px 14px", borderRadius: 8, background: "var(--card)",
                        border: `1px solid ${k.status === "claimed" ? "rgba(52,196,114,0.2)" : "var(--border)"}`,
                      }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                          <span style={{ fontFamily: "var(--mono)", fontSize: 12, fontWeight: 600, color: k.status === "claimed" ? "var(--green)" : "var(--accent)", letterSpacing: 0.5 }}>
                            {k.key_code}
                          </span>
                          <span style={{
                            fontSize: 9, fontFamily: "var(--mono)", padding: "2px 7px", borderRadius: 5,
                            textTransform: "uppercase", letterSpacing: 0.5,
                            background: k.status === "claimed" ? "rgba(52,196,114,0.08)" : k.status === "issued" ? "rgba(232,138,58,0.08)" : "var(--card)",
                            color: k.status === "claimed" ? "var(--green)" : k.status === "issued" ? "#e88a3a" : "var(--sub)",
                            border: `1px solid ${k.status === "claimed" ? "rgba(52,196,114,0.15)" : k.status === "issued" ? "rgba(232,138,58,0.15)" : "var(--border)"}`,
                          }}>
                            {k.status}
                          </span>
                        </div>
                        <div style={{ fontSize: 11, fontFamily: "var(--mono)", color: "var(--sub)", display: "flex", gap: 12, flexWrap: "wrap" }}>
                          ...
                          {k.claimed_by && <span style={{ color: "var(--green)" }}>→ {k.claimed_by}</span>}
