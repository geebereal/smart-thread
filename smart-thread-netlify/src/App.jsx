...
export default function SmartThread() {
  ...
  const [adminStats, setAdminStats] = useState(null); // { totalUsers, activeUsers, totalThreads }
  ...
  useEffect(() => {
    if (view === "settings" && isAdmin) { loadAdminStats(); loadPendingRewards(); }
  }, [view, isAdmin]);
  ...
  async function loadPendingRewards() {
    if (!supabase || !isAdmin) return;
    try {
      const { data } = await supabase.from("reward_claims").select("*, user_accounts(email)").eq("status", "pending").order("created_at", { ascending: false });
  ...
  async function loadCompletedRewards() {
    ...
    try {
      ...
      const { data } = await supabase.from("reward_claims").select("task_id, status, created_at").eq("user_id", userId);
      // Only count today's claims as done/pending
      ...
      // Count all-time approved bonus gens
      const totalBonus = (data || []).filter(r => r.status === "approved").reduce((sum, r) => sum + (REWARD_TASKS.find(t => t.id === r.task_id)?.reward || 0), 0);
