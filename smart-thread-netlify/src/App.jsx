...
const PET_PEEVES = [
  "No emojis", "Corporate jargon", "Too many questions", "Generic advice",
...
const WRITING_STYLES = [
  "Casual", "Professional", "Storytelling", "Educational", "Inspirational",
  "Witty", "Direct", "Conversational", "Authoritative", "Raw & unfiltered",
];
...
const PQ = [
  ...
  { key: "style", q: "Writing style", why: "Pick the style that best fits your brand. This shapes sentence structure, word choice, and energy.", ph: "e.g. Casual, storytelling", req: false, chips: true },
...
export default function SmartThread() {
  ...
  const [topicWhat, setTopicWhat] = useState("");
  ...
  useEffect(() => {
    ...
    if (syncing) return;
    const p = profiles;
    if (p.length > 0 && !activeProfile) setActiveProfile(p[0]);
  ...
  async function generate() {
    ...
    if (prof) {
      ...
      if (prof.style) voiceStr += `\n- Writing style: ${prof.style}`;
    ...
    const prompt = [
      `Write a Twitter/X thread about: "${topicCombined}"`,
      hookStr,
  ...
  // Build topic from guided fields
  const topicCombined = [topicWhat, topicWhy, topicAngle].filter(Boolean).join(". ");
  const ready = topicWhat.trim() && (useHook ? hookId : true) && threadLen && ctaType;
              ...
              {PQ[obStep].chips && (
                <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 12 }}>
                  {WRITING_STYLES.map(s => {
                    const sel = (obData.style || "").toLowerCase().includes(s.toLowerCase());
                    return (
                      <button key={s} onClick={() => {
                        if (sel) {
                          const r = new RegExp(`,?\\s*${s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`, "gi");
                          setObData({ ...obData, style: (obData.style || "").replace(r, "").replace(/^,\s*/, "").trim() });
                        } else {
                          setObData({ ...obData, style: obData.style ? `${obData.style}, ${s}` : s });
                        }
                      }} style={{
                        padding: "5px 11px", borderRadius: 7, fontSize: 11, fontFamily: "var(--ui)", fontWeight: 500, cursor: "pointer",
                        border: `1.5px solid ${sel ? "var(--accent)" : "var(--border)"}`,
        ...
        {view === "home" && (
          <div className="fi">
            ...
            {activeProfile ? (
              ...
              <div className="glass" style={{ borderRadius: 12, padding: "16px 18px", marginBottom: 20, borderColor: "rgba(212,85,58,0.15)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ width: 36, height: 36, borderRadius: 10, background: "var(--card)", border: "1.5px dashed var(--border)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    <span style={{ fontSize: 16, color: "var(--sub)" }}>+</span>
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13, fontWeight: 600, fontFamily: "var(--ui)" }}>Create a profile for better results</div>
                    <div style={{ fontSize: 11, fontFamily: "var(--ui)", color: "var(--sub)", marginTop: 2 }}>Set your niche, voice, and audience so every thread sounds like you.</div>
                  </div>
                  <button onClick={() => { setEditingProfileId(null); setObData({}); setObStep(0); setView("onboarding"); }} style={{
                    padding: "7px 14px", borderRadius: 8, fontSize: 12, fontFamily: "var(--ui)", fontWeight: 600,
                    border: "none", background: "linear-gradient(135deg, #d4553a, #e88a3a)", color: "#fff",
                    cursor: "pointer", flexShrink: 0, whiteSpace: "nowrap",
                  }}>Set up</button>
                </div>
            ...
            <div style={{ display: "flex", gap: 10, marginBottom: 24 }}>
              {[
                { used: gens, total: planGenLimit, label: "Generates", icon: "✦", isInf: planGenLimit === Infinity },
                { used: rw, total: planRwLimit, label: "Rewrites", icon: "↻", isInf: planRwLimit === Infinity },
              ].map(bar => {
                const left = bar.isInf ? Infinity : Math.max(0, bar.total - bar.used);
                const pct = bar.isInf ? 100 : (bar.total > 0 ? (left / bar.total) * 100 : 0);
                const low = !bar.isInf && left <= 2;
                const empty = !bar.isInf && left === 0;
                return (
                  <div key={bar.label} style={{
                    flex: 1, padding: "14px 16px", borderRadius: 12,
                    background: "var(--card)", border: `1.5px solid ${empty ? "rgba(212,85,58,0.2)" : "var(--border)"}`,
                  }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                      <span style={{ fontSize: 16 }}>{bar.icon}</span>
                      <span style={{ fontSize: 11, fontFamily: "var(--mono)", color: "var(--sub)", textTransform: "uppercase", letterSpacing: 0.5 }}>{bar.label}</span>
                    </div>
                    <div style={{ display: "flex", alignItems: "baseline", gap: 4, marginBottom: 8 }}>
                      <span style={{
                        fontSize: 28, fontWeight: 800, fontFamily: "var(--ui)", lineHeight: 1,
                        color: empty ? "var(--accent)" : low ? "#e88a3a" : "var(--text)",
                      }}>
                        {bar.isInf ? "∞" : left}
                      </span>
                      {!bar.isInf && (
                        <span style={{ fontSize: 12, fontFamily: "var(--mono)", color: "var(--sub)" }}>/ {bar.total}</span>
                      )}
                    </div>
                    {/* Progress ring */}
                    <div style={{ height: 4, borderRadius: 4, background: "var(--border)", overflow: "hidden" }}>
                      <div style={{
                        ...
                        background: bar.isInf
                          ? "linear-gradient(90deg, #34c472, #2ea85c)"
                          : empty ? "linear-gradient(90deg, #d4553a, #e88a3a)"
                          : low ? "linear-gradient(90deg, #e88a3a, #f0b429)"
                          : "linear-gradient(90deg, #34c472, #2ea85c)",
                      }} />
                    </div>
                  </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 28 }}>
              ...
              <div>
                <div style={{ fontSize: 14, fontWeight: 700, fontFamily: "var(--ui)", marginBottom: 12 }}>Topic</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  <div>
                    <div style={{ fontSize: 11, fontFamily: "var(--ui)", color: "var(--sub)", marginBottom: 4 }}>What's the main idea?</div>
                    <input value={topicWhat} onChange={e => { setTopicWhat(e.target.value); setTopic(e.target.value); }} placeholder="e.g. Why most people fail at content creation" style={{ width: "100%", padding: "10px 14px", borderRadius: 9, background: "var(--card)", border: "1.5px solid var(--border)", color: "var(--text)", fontSize: 14, fontFamily: "var(--ui)" }} />
                  </div>
                  <div>
                    <div style={{ fontSize: 11, fontFamily: "var(--ui)", color: "var(--sub)", marginBottom: 4 }}>Why does it matter? (optional)</div>
                    <input value={topicWhy} onChange={e => setTopicWhy(e.target.value)} placeholder="e.g. People waste months without seeing growth" style={{ width: "100%", padding: "10px 14px", borderRadius: 9, background: "var(--card)", border: "1.5px solid var(--border)", color: "var(--text)", fontSize: 14, fontFamily: "var(--ui)" }} />
                  </div>
                  <div>
                    <div style={{ fontSize: 11, fontFamily: "var(--ui)", color: "var(--sub)", marginBottom: 4 }}>Your unique angle? (optional)</div>
                    <input value={topicAngle} onChange={e => setTopicAngle(e.target.value)} placeholder="e.g. I grew from 0 to 10K by doing the opposite" style={{ width: "100%", padding: "10px 14px", borderRadius: 9, background: "var(--card)", border: "1.5px solid var(--border)", color: "var(--text)", fontSize: 14, fontFamily: "var(--ui)" }} />
                  </div>
                </div>
              </div>
              ...
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                  ...
                  <button onClick={() => {
                    ...
                    const t = topicCombined.trim().toLowerCase();
